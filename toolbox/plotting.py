...
def plotting(
    ...
    # --- 1. Write slot IDs to MP(output).csv ---
    MP['Slot_IDs'] = ['']*P  # Initialize an empty column for slot IDs
    for l in range(L):
        for p in range(P):
            if sol.get_value(x[l, p]) == 1:
                MP['Slot_IDs'][p] += f"{l+1}, "
        ...
        for l in zone:
            for p in range(P):
                if sol.get_value(x[l-1, p]) == 1:
                    MP[zone_name][p] += int(sol.get_value(y[l-1, p]))

    # Save data frame MP into a .csv
    MP.to_csv("./products_data(output).csv", index=True, header=True)
    ...
    a_class_products = [allocation for allocation in product_allocation if MP['Class'][allocation[0]] == 'A']
    ...
    a_prod = [[allocation[1], allocation[0], allocation[2], MP['Class'][allocation[0]], 'blue'] for allocation in a_class_products]

    b_class_products = [allocation for allocation in product_allocation if MP['Class'][allocation[0]] == 'B']
    ...
    b_prod = [[allocation[1], allocation[0], allocation[2], MP['Class'][allocation[0]], 'yellow'] for allocation in b_class_products]

    c_class_products = [allocation for allocation in product_allocation if MP['Class'][allocation[0]] == 'C']
    ...
    c_prod = [[allocation[1], allocation[0], allocation[2], MP['Class'][allocation[0]], 'purple'] for allocation in c_class_products]
        ...
        for column_label, column_coords in column_data.items():  # Iterate using column labels
            for j in range(len(occupied_slots)):
                if occupied_slots[j][0] in column_coords:
                    ...
                    if product_class == 'A':  # Use a_class_products for class A
                        aoccupied_text.append(f"Slot: {occupied_slots[j][0]}<br>Class: {occupied_slots[j][3]}<br>Product Index: {occupied_slots[j][1]+1}<br>#LC: {occupied_slots[j][2]}<br>Code: {slot_code}")
                    elif product_class == 'B':  # Use a_class_products for class A
                        boccupied_text.append(f"Slot: {occupied_slots[j][0]}<br>Class: {occupied_slots[j][3]}<br>Product Index: {occupied_slots[j][1]+1}<br>#LC: {occupied_slots[j][2]}<br>Code: {slot_code}")
                    elif product_class == 'C':  # Use a_class_products for class A
                        coccupied_text.append(f"Slot: {occupied_slots[j][0]}<br>Class: {occupied_slots[j][3]}<br>Product Index: {occupied_slots[j][1]+1}<br>#LC: {occupied_slots[j][2]}<br>Code: {slot_code}")
